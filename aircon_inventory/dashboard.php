...
<head>
  ...
  <title>Admin Dashboard - Aircon Inventory</title>
...
<body class="bg-gray-100 flex min-h-screen">
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
              <tr class="hover:bg-gray-50">
                ...
                <td class="py-2 px-4 border text-center space-x-2">
                  <button onclick="viewProduct(<?= $row['id'] ?>)" class="bg-blue-500 px-3 py-1 rounded hover:bg-blue-600 text-sm text-white">View</button>
                  <a href="edit_product.php?id=<?= $row['id'] ?>" class="bg-yellow-400 px-3 py-1 rounded hover:bg-yellow-500 text-sm text-white">Edit</a>
                  <a href="archive_product.php?id=<?= $row['id'] ?>" onclick="return confirm('Archive this product?')" class="bg-gray-600 px-3 py-1 rounded hover:bg-gray-700 text-sm text-white">Archive</a>
                  <a href="delete_product.php?id=<?= $row['id'] ?>" onclick="return confirm('Are you sure you want to delete this product permanently?')" class="bg-red-600 px-3 py-1 rounded hover:bg-red-700 text-sm text-white">Delete</a>
                </td>
              </tr>
            <?php endwhile; ?>
          <?php else: ?>
  ...
  <div id="addProductModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
      <h3 class="text-2xl font-bold mb-4 text-blue-700">Add New Product</h3>

      <form action="add_product.php" method="POST" class="space-y-4">
        <div>
          <label class="block font-medium">Product Name</label>
          <input type="text" name="product_name" required class="w-full border rounded px-3 py-2 mt-1 focus:ring focus:ring-blue-300">
        </div>

        <div>
          <label class="block font-medium">Category</label>
          <input type="text" name="category" required class="w-full border rounded px-3 py-2 mt-1 focus:ring focus:ring-blue-300">
        </div>

        <div>
          <label class="block font-medium">Brand</label>
          <input type="text" name="brand" class="w-full border rounded px-3 py-2 mt-1 focus:ring focus:ring-blue-300">
        </div>

        <div>
          <label class="block font-medium">Description</label>
          <textarea name="description" rows="3" class="w-full border rounded px-3 py-2 mt-1 focus:ring focus:ring-blue-300"></textarea>
        </div>

        <div class="flex gap-4">
          <div class="flex-1">
            <label class="block font-medium">Stock</label>
            <input type="number" name="stock" min="0" required class="w-full border rounded px-3 py-2 mt-1 focus:ring focus:ring-blue-300">
          </div>
          <div class="flex-1">
            <label class="block font-medium">Price (₱)</label>
            <input type="number" step="0.01" name="price" required class="w-full border rounded px-3 py-2 mt-1 focus:ring focus:ring-blue-300">
          </div>
        </div>
  ...
  <div id="viewModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
      <h3 class="text-2xl font-bold mb-4 text-blue-700">Product Details</h3>
